...
const ImageItem: FC<ImageItemProps> = ({ image, onChange }) => {
  ...
  return (
    <li className="w-1/2 sm:w-1/3 lg:w-1/4 pl-2 pr-2 pb-4" onClick={handleClick}>
      <div className="group relative rounded-md block h-24 bg-gray-100 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-white-100 focus-within:ring-blue-500 overflow-hidden cursor-pointer">
        <img
    ...
    <ul className="flex flex-wrap -ml-2 -mr-2">
      {Array.from({ length: 12 }).map((_, index) => (
        <li key={index} className="w-1/2 sm:w-1/3 lg:w-1/4 pl-2 pr-2 pb-4">
          <div className="skeleton block h-24 rounded-md" />
        </li>
      ))}
    </ul>
  )
