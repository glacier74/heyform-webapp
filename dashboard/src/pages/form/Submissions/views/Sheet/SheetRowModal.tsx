...
import { Button, Input, Tooltip } from '@heyforms/ui'
...
const DateItem: FC<SheetCellProps> = ({ column, row }) => {
  return (
    <div className="min-h-11 w-[240px] cursor-not-allowed whitespace-pre-line rounded-lg bg-white p-[10px] text-sm font-normal leading-6 text-[#4e5d78]">
      {row[column.key]}
    </div>
  )
}
...
const InputTableItem: FC<SheetCellProps> = ({ column, row }) => {
  ...
  return (
    <div className="min-h-11 cursor-not-allowed whitespace-pre-line border border-[#f3f3f3] bg-white p-2 text-sm font-normal leading-6 text-[#4e5d78]">
  ...
  return (
    <div className="min-h-11 cursor-not-allowed whitespace-pre-line border border-[#f3f3f3] bg-white p-2 text-sm font-normal leading-6 text-[#4e5d78]">
      {isValid(choices) ? (
        ...
        <div className="min-h-20 flex items-center justify-center text-[#A1A1A1]">
          Not been answered
        </div>
      )}
    </div>
...
const MultipleChoiceItem: FC<SheetCellProps> = ({ column, row }) => {
  ...
  return (
    <div className="min-h-11 cursor-not-allowed whitespace-pre-line rounded-lg bg-white p-[10px] text-sm font-normal leading-6 text-[#4e5d78]">
      {isValid(choices) && <TagGroup tags={choices as any} />}
    </div>
  )
}
...
export const SheetRowModal: FC<SheetRowModalProps> = ({
  ...
  return (
    <>
      {visible && (
        <div className="heygrid-panel fixed top-0 right-0 bottom-0 left-0 z-10">
          <div
            className="absolute top-0 right-0 bottom-0 left-0 z-[10] bg-[rgba(50,59,75,0.3)]"
            onClick={onClose}
          />
          <div className="absolute top-0 right-[0px] bottom-0 z-[100] w-[40%] max-w-[600px] rounded-tl-xl rounded-bl-xl bg-white/90 shadow-2xl backdrop-blur-3xl">
            ...
            <div className="h-[calc(100vh-3.75rem)] overflow-y-auto px-9 pt-9 pb-6">
