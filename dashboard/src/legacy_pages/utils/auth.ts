...
export function getDangerZoneState() {
  return cookies.get(dangerZoneKey)
}

export function setDangerZoneState(value: string) {
  cookies.set(dangerZoneKey, value, {
    ...cookieOptions,
    expires: date().add(1, 'hour').toDate()
  })
}

export function setWorkspaceId(workspaceId: string) {
  cookies.set(workspaceIdKey, workspaceId, cookieOptions)
}
