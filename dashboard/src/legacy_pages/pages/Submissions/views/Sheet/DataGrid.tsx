import FilterRow from '@/legacy_pages/pages/Submissions/views/Sheet/FilterRow'
import GroupRowRenderer from '@/legacy_pages/pages/Submissions/views/Sheet/GroupRow'
import HeaderRow from '@/legacy_pages/pages/Submissions/views/Sheet/HeaderRow'
...
import {
  ...
  useViewportColumns,
  useViewportRows
} from '@/legacy_pages/pages/Submissions/views/Sheet/hooks'
import Row from '@/legacy_pages/pages/Submissions/views/Sheet/Row'
import SummaryRow from '@/legacy_pages/pages/Submissions/views/Sheet/SummaryRow'
...
import {
  ...
  onEditorNavigation
} from '@/legacy_pages/pages/Submissions/views/Sheet/utils'
import { Spin } from '@heyui/component'
...
export interface DataGridProps<R, SR = unknown> extends SharedDivProps {
  ...
  onRowExpand: (idx: number, row: Readonly<R>) => void
...
function DataGrid<R, SR>(
  ...
  function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    ...
    const header: HTMLDivElement = document.querySelector('.heygrid-header-row')!
    const pane: HTMLDivElement = header.querySelector('.heygrid-header-frozen-pane')!
    header.style.transform = `translate3d(-${scrollLeft}px, 0, 0)`
    pane.style.transform = `translate3d(${scrollLeft}px, 0, 0)`
  ...
  return (
    <GridRoot
      className={clsx('heygrid', { 'heygrid-viewport-dragging': isDragging }, className)}
      style={
        {
          ...style,
    ...
    >
      <HeaderRow<R, SR>
        ...
        allRowsSelected={rawRows.length > 0 && selectedRows?.size === rawRows.length}
        ...
        <div className="heygrid-scrollview" onScroll={handleScroll}>
          <div
            style={{
              ...
              height: Math.max(rows.length * rowHeight, clientHeight) + (loading ? rowHeight : 0)
            }}
          />

          <div className="heygrid-coreaera">
            <div
              ref={focusSinkRef}
              tabIndex={0}
              className="heygrid-focus-sink"
              onKeyDown={handleKeyDown}
              onFocus={onGridFocus}
            />
            {getViewportRows()}
...
const GridRoot = styled.div`
  ...
  .heygrid-group-row-selected > .heygrid-cell:first-child {
    box-shadow: inset 2px 0 0 0 ${props => props.theme.primary};
  }
...
export default forwardRef(DataGrid) as <R, SR = unknown>(
  props: DataGridProps<R, SR> & RefAttributes<DataGridHandle>
) => JSX.Element
