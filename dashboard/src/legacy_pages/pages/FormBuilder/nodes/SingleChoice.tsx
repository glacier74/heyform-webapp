...
import { InlineText, PseudoRadio } from '../components'
import { useStoreContext } from '../store'
import { getFieldFromKind, placeCaretAtEnd } from '../utils'
import { Block, BlockProps } from './Block'

export const SingleChoice: FC<BlockProps> = ({ field, ...restProps }) => {
    ...
    >
      <div>
        {field.properties?.choices?.map((choice, index) => (
          <StyledPseudoRadio key={choice.id} contentEditable={undefined}>
            <StyledInlineText
              index={index}
              value={choice.label}
...
const StyledPseudoRadio = styled(PseudoRadio)`
  margin-top: 12px;
